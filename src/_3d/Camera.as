...
//			
		...
		public function clip( a:Vector3D, b:Vector3D ):Vector.<Vector3D>
		{
			...
			{
				for ( var j:int = 0; j < _planes.length; j++ )
				{
					var plane:Plane = _planes[j] ;
					var adistance:Number = plane.test( result[i] );
					var bdistance:Number = plane.test( result[(i+1)%2] );
				...
				if ( adistance < 0 && bdistance < 0 )
				{
					//	If they're both outside of any plane, return nothing
					return null ;
		...
		public function lookAt( point:Vector3D, worldUp:Vector3D ):Matrix4x4 
		{
			...
			//	Compute the difference between the world
			//	up vector and the direction vector's projection
			//	onto it	
			_up = _direction.crossProduct( worldUp ) ;
